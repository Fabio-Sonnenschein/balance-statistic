...
budgetRouter.post('', async (req: Request, res: Response, next: NextFunction) => {
  ...
  let insert = await req.services.budgetService.insertBudget(user._id, req.body.budget)
    .catch((error: Error) => {
      return next(error);
    });
  if (!insert) return next(new HttpException('Budget update failed', 500));
...
budgetRouter.put('/savingRate', async (req: Request, res: Response, next: NextFunction) => {
  ...
  let user: User = await req.services.authService.authToken(req.body.token, req).catch((error: Error) => {
    return next(error);
  });
  if (!user) return next(new HttpException('Request unauthorized', 401));

  let update = await req.services.budgetService.updateBudgetSavingRate(user._id, req.body.budget.savingRate)
    .catch((error: Error) => {
      return next(error);
    });
  if (!update) return next(new HttpException('Budget update failed', 500));
  res.status(200).json(true);
});
...
budgetRouter.put('/budget', async (req: Request, res: Response, next: NextFunction) => {
  ...
  let update = await req.services.budgetService.updateBudgetBudget(user._id, req.body.budget.budget)
    .catch((error: Error) => {
      return next(error);
    });
  if (!update) return next(new HttpException('Budget update failed', 500));
...
budgetRouter.put('/earnings', async (req: Request, res: Response, next: NextFunction) => {
  ...
  let user: User = await req.services.userService.authToken(req.body.token, req)
    .catch((error: Error) => {
      return next(error);
    });
  if (!user) return next(new HttpException('Request unauthorized', 401));

  let update = await req.services.budgetService.updateBudgetEarnings(user._id, req.body.budget.earnings)
    .catch((error: Error) => {
      return next(error);
    });
  if (!update) return next(new HttpException('Budget update failed', 500));
...
budgetRouter.put('/expenses', async (req: Request, res: Response, next: NextFunction) => {
  ...
  let update = await req.services.budgetService.updateBudgetExpenses(user._id, req.body.buget.expenses)
    .catch((error: Error) => {
      return next(error);
    });
  if (!update) return next(new HttpException('Budget update failed', 500));
...
budgetRouter.delete('', async (req: Request, res: Response, next: NextFunction) => {
  ...
  let insert = await req.services.budgetService.insertBudget(user._id, emptyBudget)
    .catch((error: Error) => {
      return next(error);
    });
  if (!insert) return next(new HttpException('Budget removal failed', 500));
  res.status(200).send();
});
